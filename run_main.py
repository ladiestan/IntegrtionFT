...
def run_case(all_case, reportName="report"):
    ...
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title=u"自动化测试接口", description=u"用例执行情况")
    # 调用add_case函数返回值
    runner.run(all_case)
    fp.close()


# 此处为了获取测试报告，不一定正确，需要调试
...
if __name__ == "__main__":
    ...
    report_file = get_report_file(report_path)  # 3获取测试报告
# 邮箱配置
